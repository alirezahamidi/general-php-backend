<?php
class CommericalOrdering
{
    function __construct($config)
    {
        $this->mysql = new MySql($config->db->list->ordering, $config);
    }

    function addOrder($data, $config)
    {
        $conn = $this->mysql->getConnection();
        $now = new DateTime("now");
        $query = $this->mysql->buildAddQuery("orders", ["id", "user", "source", "type", "typeDescription", "title", "description", "phone", "date"], [null, $config->catch->userData->id, "commerical_web", $data->type, $data->typeDescription, $data->title, $data->description, $data->phone, $now->format('Y-m-d H:i')]);
        ...
        switch ($name) {
            case "add":
                if (isset($data) && isset($data->data)) {
                    $this->addOrder($data->data, $config);
                } else {
...
$login = new CommericalOrdering($config);
